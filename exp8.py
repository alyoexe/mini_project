...
LIME_QUALITY_MODE = "balanced"  # "fast" = simple, "balanced" = medium, "high" = detailed
...
ENABLE_HITL_FEEDBACK = True
FEEDBACK_ROOT = "feedback_data"
FEEDBACK_LOG = "feedback_data/feedback_log.jsonl"
FEEDBACK_CROP_DIR = "feedback_data/crops"
FEEDBACK_PROMPT_TIMEOUT = 6.0  # Auto-close prompt if no action.
FEEDBACK_PROMPT_COOLDOWN = 10.0  # Delay before asking the same track again.
FEEDBACK_BUTTON_H = 26
FEEDBACK_TILE_FOOTER_H = 56
FEEDBACK_BUTTON_PAD = 8
# ==========================================
# UTILITY FUNCTIONS
# ==========================================


def ensure_feedback_paths():
    root = Path(FEEDBACK_ROOT)
    crop_dir = Path(FEEDBACK_CROP_DIR)
    log_path = Path(FEEDBACK_LOG)
    root.mkdir(parents=True, exist_ok=True)
    crop_dir.mkdir(parents=True, exist_ok=True)
    if not log_path.exists():
        log_path.touch()
    return log_path, crop_dir


def load_feedback_prompt_ids(log_path):
    seen_ids = set()
    try:
...
def append_feedback_record(log_path, record):
    with log_path.open("a", encoding="utf-8") as f:
        f.write(json.dumps(record, ensure_ascii=True) + "\n")


def safe_save_feedback_crop(frame_bgr, bbox, crop_dir, prompt_id):
    ...
    if crop.size == 0:
        return ""
    out_path = crop_dir / f"{prompt_id}.jpg"
    ok = cv2.imwrite(str(out_path), crop)
    return str(out_path) if ok else ""


def draw_hitl_stats(frame, stats_line):
    cv2.putText(
        frame,
        stats_line,
        (10, frame.shape[0] - 10),
...
def on_lime_mouse(event, x, y, _flags, state):
    ...
    for entry in buttons:
        ...
        if _inside_rect(x, y, rect):
            state["click"] = (entry.get("obj_id"), entry.get("decision"))
            break
...
def main():
    ...
    feedback_log_path, feedback_crop_dir = ensure_feedback_paths()
    seen_prompt_ids = load_feedback_prompt_ids(feedback_log_path)
    feedback_stats = {
        "prompts": 0,
        "yes": 0,
        "no": 0,
    ...
    last_obj_meta = {}
    prompted_this_streak = set()
    prompt_cooldown_until = {}
    pending_feedback = {}
    lime_mouse_state = {
        "buttons": [],
        "click": None,
    ...
    print("HITL controls: click YES/NO in each object tile inside the LIME window")
    print("Running...")

    cv2.namedWindow("Detection")
    cv2.namedWindow("LIME")
    cv2.setMouseCallback("LIME", on_lime_mouse, lime_mouse_state)

    def close_prompt_for_obj(obj_id, decision):
        prompt = pending_feedback.get(obj_id)
        if prompt is None:
            return
        ...
        record = {
            ...
            "prompt_id": prompt_id,
            "decision": decision,
            "model_correct": decision == "yes",
            "object_id": obj_id,
            "frame_index": int(prompt.get("frame_index", -1)),
            "class_id": prompt_meta.get("cls", -1),
            "class_name": prompt_meta.get("name", "unknown"),
            "confidence": round(float(prompt_meta.get("conf", 0.0)), 4),
            "bbox": prompt_meta.get("bbox", [0, 0, 0, 0]),
            "crop_path": prompt.get("crop_path", ""),
        }

        try:
            append_feedback_record(feedback_log_path, record)
            seen_prompt_ids.add(prompt_id)
        except Exception:
            feedback_stats["write_errors"] += 1

        feedback_stats["prompts"] += 1
        if decision in ("yes", "no", "skip"):
            feedback_stats[decision] += 1

        prompt_cooldown_until[obj_id] = time.time() + FEEDBACK_PROMPT_COOLDOWN
        pending_feedback.pop(obj_id, None)
    ...
    try:
                ...
                for r in results:
                    ...
                    # ---------- UNCERTAIN ----------
                        ...
                        if LOWER_THRESH <= conf <= UPPER_THRESH:
                            ...
                            prompted_this_streak.discard(obj_id)
            ...
            # remove lost ids
                ...
                for k in list(uncertain_times.keys()):
                    if k not in current_ids:
                        ...
                        prompted_this_streak.discard(k)
                        last_obj_meta.pop(k, None)
            ...
            # -------- async LIME --------
                ...
                while True:
                    try:
                        ...
                        # Queue a human prompt per object when uncertainty explanation is ready.
                        if ENABLE_HITL_FEEDBACK:
                            meta = last_obj_meta.get(obj_id)
                            cooldown_until = prompt_cooldown_until.get(obj_id, 0.0)
                            if (
                                meta is not None
                                and LOWER_THRESH <= float(meta.get("conf", 0.0)) <= UPPER_THRESH
                                and obj_id not in prompted_this_streak
                                and now_exp >= cooldown_until
                                and obj_id not in pending_feedback
                            ):
                                prompt_id = f"{int(now_exp * 1000)}_{obj_id}_{int(meta.get('cls', -1))}"
                                if prompt_id not in seen_prompt_ids:
                                    crop_path = safe_save_feedback_crop(
                                        raw_frame,
                                        meta.get("bbox", [0, 0, 0, 0]),
                                        feedback_crop_dir,
                                        prompt_id,
                                    )
                                    pending_feedback[obj_id] = {
                                        "started_at": now_exp,
                                        "prompt_id": prompt_id,
                                        "frame_index": frame_count,
                                        "meta": {
                                            "conf": float(meta.get("conf", 0.0)),
                                            "cls": int(meta.get("cls", -1)),
                                            "name": str(meta.get("name", "unknown")),
                                            "bbox": [int(v) for v in meta.get("bbox", [0, 0, 0, 0])],
                                        },
                                        "crop_path": crop_path,
                                    }
                                    prompted_this_streak.add(obj_id)
                    except queue.Empty:
                        break
        ...
        # -------- FPS --------
            ...
            if ENABLE_HITL_FEEDBACK:
                stats_line = (
                    f"HITL prompts {feedback_stats['prompts']}"
                    f" | yes {feedback_stats['yes']}"
                    f" | no {feedback_stats['no']}"
                    f" | skip {feedback_stats['skip']}"
                    f" | pending {len(pending_feedback)}"
                    f" | write_err {feedback_stats['write_errors']}"
                )
                draw_hitl_stats(frame, stats_line)
        ...
        # Show a single tiled window to avoid opening many OS windows (reduces lag).
            if runtime_show_lime_window and pinned_order:
                ...
                if active_ids:
                    ...
                    rows = 1
                cell_h = tile_size + FEEDBACK_TILE_FOOTER_H
                panel = np.zeros((rows * cell_h, cols * tile_size, 3), dtype=np.uint8)
                lime_mouse_state["buttons"] = []
                ...
                for i, obj_id in enumerate(active_ids[:MAX_LIME_TILES]):
                    ...
                    meta = last_obj_meta.get(obj_id, {})
                    name = str(meta.get("name", "obj"))
                    cv2.putText(
                        panel,
                        f"ID {obj_id} {name} {max(0.0, EXPLANATION_HOLD_SEC + EXPLANATION_FADE_SEC - age):.1f}s",
                        (x0 + 8, y0 + 20),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.43,
                        (255, 255, 255),
                    ...
                    if ENABLE_HITL_FEEDBACK and obj_id in pending_feedback:
                        footer_top = y0 + tile_size + FEEDBACK_BUTTON_PAD
                        half = (tile_size - 3 * FEEDBACK_BUTTON_PAD) // 2
                        yes_rect = (
                            x0 + FEEDBACK_BUTTON_PAD,
                            footer_top,
                            x0 + FEEDBACK_BUTTON_PAD + half,
                            footer_top + FEEDBACK_BUTTON_H,
                        )
                        no_rect = (
                            x0 + 2 * FEEDBACK_BUTTON_PAD + half,
                            footer_top,
                            x0 + 2 * FEEDBACK_BUTTON_PAD + 2 * half,
                            footer_top + FEEDBACK_BUTTON_H,
                        )

                        cv2.rectangle(panel, (yes_rect[0], yes_rect[1]), (yes_rect[2], yes_rect[3]), (35, 145, 35), -1)
                        cv2.rectangle(panel, (yes_rect[0], yes_rect[1]), (yes_rect[2], yes_rect[3]), (255, 255, 255), 1)
                        cv2.putText(panel, "YES", (yes_rect[0] + 18, yes_rect[1] + 18), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

                        cv2.rectangle(panel, (no_rect[0], no_rect[1]), (no_rect[2], no_rect[3]), (35, 35, 170), -1)
                        cv2.rectangle(panel, (no_rect[0], no_rect[1]), (no_rect[2], no_rect[3]), (255, 255, 255), 1)
                        cv2.putText(panel, "NO", (no_rect[0] + 24, no_rect[1] + 18), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

                        remaining = FEEDBACK_PROMPT_TIMEOUT - (now - pending_feedback[obj_id]["started_at"])
                        cv2.putText(
                            panel,
                            f"Decide in {max(0.0, remaining):.1f}s",
                            (x0 + 8, footer_top + FEEDBACK_BUTTON_H + 16),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.42,
                            (0, 255, 255),
                            1,
                        ...
                        lime_mouse_state["buttons"].append({"obj_id": obj_id, "decision": "yes", "rect": yes_rect})
                        lime_mouse_state["buttons"].append({"obj_id": obj_id, "decision": "no", "rect": no_rect})
                    elif ENABLE_HITL_FEEDBACK:
                        cv2.putText(
                            panel,
                            "Decision recorded",
                            (x0 + 8, y0 + tile_size + 24),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.44,
                            (120, 220, 120),
                            1,
            ...
            if ENABLE_HITL_FEEDBACK:
                click = lime_mouse_state.get("click")
                if click is not None:
                    clicked_obj_id, clicked_decision = click
                    if clicked_decision in ("yes", "no"):
                        close_prompt_for_obj(clicked_obj_id, clicked_decision)
                    lime_mouse_state["click"] = None

                now_decision = time.time()
                expired_ids = [
                    oid for oid, prompt in pending_feedback.items()
                    if now_decision - prompt.get("started_at", now_decision) >= FEEDBACK_PROMPT_TIMEOUT
                ]
                for oid in expired_ids:
                    close_prompt_for_obj(oid, "skip")
