...
VIDEO_SOURCE = "test_video.mp4"   # <<< change to your video
...
def main():
    ...
    while cap.isOpened():
        ...
        if uncertain_found:
            ...
            if uncertain_start is None:
                uncertain_start = now

            duration = now - uncertain_start

            if (
                duration > UNCERTAIN_TIME
                and now - last_lime_time > LIME_COOLDOWN
                and uncertain_crop is not None
                and input_q.empty()
            ):

                input_q.put(
                    (uncertain_crop, uncertain_cls)
                )
