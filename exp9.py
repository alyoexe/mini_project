...
BATCH_FLUSH_SEC = 0.1  # Flush partial batch after this delay
NUM_LIME_WORKERS = 4  # Number of parallel LIME workers (1-4 recommended, higher = faster but more GPU/CPU)
...
MAX_LIME_JOBS_PER_SEC = 5.0  # Global cap on new LIME jobs across all objects
    ...
    def process_single(crop, cls, obj_id):
        ...
        try:
            ...
            if not ENABLE_MASK_OVERLAY:
                ...
                # Use the segmented image (temp) as the base instead of crop
                # This shows which regions LIME identified
                if LIME_POSITIVE_ONLY:
                    mask_u8 = (mask > 0).astype(np.uint8) * 255
                    if ENABLE_MASK_SMOOTH:
                        mask_u8 = cv2.GaussianBlur(mask_u8, (5, 5), 2)
                    heat = cv2.applyColorMap(mask_u8, cv2.COLORMAP_JET)
                    overlay = cv2.addWeighted(crop_bgr, 0.65, heat, 0.35, 0)
                else:
                    # Create a colored heatmap from the mask values
                    # Normalize mask to full range for better visualization
                    mask_min = np.min(mask)
                    mask_max = np.max(mask)
                    mask_range = mask_max - mask_min
                    
                    if mask_range > 0:
                        mask_normalized = (mask - mask_min) / mask_range * 255
                    else:
                        mask_normalized = np.zeros_like(mask)
                    
                    mask_u8 = mask_normalized.astype(np.uint8)
                    
                    # Apply a colormap to see both positive and negative contributions
                    heat = cv2.applyColorMap(mask_u8, cv2.COLORMAP_JET)
                    overlay = cv2.addWeighted(crop_bgr, 0.55, heat, 0.45, 0)
